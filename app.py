...
UN_M49_GEO = {'AD': {'country': 'Andorra', 'intermediate': None, 'region': 'Europe', 'subregion': 'Southern Europe'},
 ...
 'AW': {'country': 'Aruba',
        ...
        'region': 'Americas',
        'subregion': 'Latin America and the Caribbean'},
 'AX': {'country': '脜land Islands', 'intermediate': None, 'region': 'Europe', 'subregion': 'Northern Europe'},
 ...
 'RE': {'country': 'R茅union', 'intermediate': 'Eastern Africa', 'region': 'Africa', 'subregion': 'Sub-Saharan Africa'},
 ...
 'TR': {'country': 'T眉rkiye', 'intermediate': None, 'region': 'Asia', 'subregion': 'Western Asia'},
...
def un_region_label(country_code: str) -> str:
    ...
    if country_code == "TW":
        return "Asia 路 Eastern Asia 路 not separately listed in the supplied UN M49 source"

    info = UN_M49_GEO.get(country_code, {})
    parts = [info.get("region"), info.get("subregion"), info.get("intermediate")]
    parts = [p for p in parts if p]
    return " 路 ".join(parts) if parts else "UN M49 classification unavailable"
...
def zone_display(tz_name: str, ref_date: date) -> str:
    return f"{friendly_zone_name(tz_name)} 路 {utc_offset_label(tz_name, ref_date)}"
...
def _time_in_window(local_t: time, start_t: time, end_t: time) -> bool:
    ...
    # Supports an overnight availability window, e.g. 22:00鈥?6:00.
...
def local_status(person: dict, local_dt: datetime):
    ...
    Outside that window, 00:00鈥?6:00 is shown as sleep time and all other hours
...
def local_clock_segments_utc(person: dict, ref_date: date):
    ...
    Outside that window, 00:00鈥?6:00 local is sleep and remaining hours are
...
def preferred_window_utc_segments(person: dict, ref_date: date):
    """Return preferred local hours as one or two segments on a 0鈥?4 UTC axis."""
    tz = ZoneInfo(person["tz_name"])
...
st.markdown(
    ...
    <div class="participant-intro-box">
        <div class="participant-intro-grid">
            <div class="participant-intro-title">Who鈥檚 joining?</div>
            <div class="participant-intro-copy">
                Add the people or teams joining the meeting and choose their country or area.
                For U.S. participants, choose the state too. Working hours default to 08:00鈥?7:00
...
for index, person in enumerate(list(people)):
    ...
    with st.container(border=True):
        ...
        participant_meta = (
            f"{location_label} 路 "
            f"{friendly_zone_name(person['tz_name'])} 路 "
...
with add_col:
    if st.button("锛?Add person", type="primary", use_container_width=True):
    ...
    offset_rows.append(
        {
            ...
            "Preferred hours": f"{p['earliest'].strftime('%H:%M')}鈥搟p['latest'].strftime('%H:%M')}",
...
with reference_col:
    reference_tz = st.selectbox(
        ...
        format_func=lambda z: (
            f"{friendly_zone_name(z)} 路 {utc_offset_label(z, meeting_date)}"
            + (" 路 your browser" if z == browser_timezone else "")
        ),
        help=(
...
with control_col:
    map_hour = st.slider(
        ...
        help="Choose the proposed meeting time in the reference time zone shown on the left. "
             "The app converts that instant to every participant鈥檚 local time.",
    )
...
for p in people:
    ...
    approx_text = " 路 approximate point" if approximate else ""
    ...
    world_fig.add_trace(
        go.Scattergeo(
            ...
            hovertemplate=(
                ...
                f"{status_label}<br>"
                f"{p['tz_name']} 路 {utc_offset_label(p['tz_name'], meeting_date)}"
...
st.markdown(
    ...
    ">
        ...
        ({reference_offset})
        路 {map_reference_utc.strftime('%H:%M')} UTC
        <br>
        <span style="color:#A8C9AE;font-weight:700;">鈼?/span> inside selected availability
        &nbsp;&nbsp;
        <span style="color:#F3D6E7;font-weight:700;">鈼?/span> awake but outside availability
        &nbsp;&nbsp;
        <span style="color:#4A4A4A;font-weight:700;">鈼?/span> 00:00鈥?6:00 local sleep hours
        &nbsp;&nbsp;
        <span style="color:#555b63;font-weight:700;">鈼?/span> shaded map area = night
    </div>
    """,
    unsafe_allow_html=True,
)
        ...
        band_fig.add_trace(
            go.Bar(
                ...
                hovertemplate=(
                    f"<b>{p['name']}</b><br>"
                    f"{label}<br>"
                    f"{friendly_zone_name(p['tz_name'])} 路 "
...
def _add_meeting_window(x0, x1, annotation=False):
    ...
    if annotation:
        kwargs["annotation_text"] = (
            ...
            f"{reference_offset} 路 {duration} min"
        )
...
for rank, (_, row) in enumerate(display_results.iterrows(), start=1):
    ...
    for p in people:
        ...
        people_parts.append(
            ...
            f' 路 {local.strftime("%a %d %b %Y")}'
            f' 路 {local.strftime("%H:%M")}'
            '<span class="ranked-proposal-meta">'
            f' 路 {place} 路 {tz_abbrev} 路 {utc_offset} 路 {status_text}'
            '</span>'
            '</div>'
        )
    ...
    card_html = (
        ...
        f'<div class="ranked-proposal-title">#{rank} 路 Score {float(row["Overall score"]):.0f}/100</div>'
...
with st.expander("Email-ready proposal 路 top 3", expanded=False):
    ...
    for proposal_rank, (_, proposal_row) in enumerate(results.head(3).iterrows(), start=1):
        ...
        proposal_lines.append(
            f"{proposal_rank}. {user_local.strftime('%a %d %b')} 鈥?"
            + " / ".join(option_parts)
        )
...
with st.expander("Full 24-hour comparison", expanded=False):
    ...
    for hour in range(24):
        ...
        for p in people:
            ...
            timeline_row[p["name"]] = (
                f"{local.strftime('%a %H:%M')} 路 "
                f"{utc_offset_label(p['tz_name'], local.date())} 路 {status}"
            )
...
st.markdown(
    ...
    <div class="dc-footer">
        ...
        Global Meeting Planner 路 Designed for professional use with clients and business partners.<br>
        ...
        Built by Drapalski Consulting LLC 路 Comments / feedback:
